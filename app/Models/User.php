...
class User extends Authenticatable implements MustVerifyEmail {
    ...
    public function getIncExpLineChartDate() {
        ...
        for ($i = 0; $i < count($arrDate); $i++) {
            ...
            $daysSalesOrders = SalesOrder::select('*')->whereRAW('sale_date = ?', $arrDate[$i])->get();
    ...
    public function usersLocationLogs() {
        return $this->hasMany('App\Models\UsersLocationLogs');
    }

    public function userCheckInCheckOutDetails() {
        return $this->hasMany('App\Models\UserCheckinCheckoutDetails');
    }

    public function salesReturnFormat($number) {
        $settings = Utility::settings();
