...
buttonClass: 'btn',
 ...
 <?php if(Auth::user()->type == 'Sales Agent'): ?>
<script>
        ...
        // When the page loads, ask for confirmation to start the alarm
        window.onload = function() {
            $.ajax({
            type: "GET",
            url: '<?php echo e(url("/leads/notify")); ?>',
            success: function (response) {                
                if(response){
                    var edit_url = '<?php echo e(url("/lead/'+ response.id+'/edit")); ?>';
                    alarmSound.play().then(() => {
                    console.log("Alarm started!");

                    // Show confirm message AFTER alarm starts (slight delay)
                    setTimeout(() => {
                        const userConfirmed = confirm("New LEAD :  Do you want to Accept ?");

                        if (userConfirmed) { 
                             $.ajax({
                                type: "GET",
                                url: '<?php echo e(url("/leads/assign")); ?>/' + response.id,
                                success: function (resp) {
                                   console.log("Done");
                                   location.href = '<?php echo e(url("/lead")); ?>/'+response.id+'/edit';
                                },
                    ...
                    }, 500); // slight delay to ensure audio starts
...
<script>
    ...
    <script>
        var toster_pos = "<?php echo e(env('SITE_RTL') =='on' ?'left' : 'right'); ?>";</script>
    ...
    <script>
        ...
        today: "<?php echo e(__('today')); ?>",
                ...
                ?>
<?php /**PATH E:\xampp2\htdocs\AS_CRM\resources\views/partials/admin/footer.blade.php ENDPATH**/ ?>